...
/*
//--------------------------------------------------------------
void ofxKinectProjectorToolkit::calibrate(vector<glm::vec3> pairsKinect, vector<glm::vec2> pairsProjector) {
...
/*
//--------------------------------------------------------------
glm::vec2 ofxKinectProjectorToolkit::getProjectedPoint(glm::vec3 worldPoint) {
...
void ofxKinectProjectorToolkit::saveCalibration(string path) {
    ...
    */
    ...
    ofXml xml;
    auto calibrationXml = xml.appendChild("CALIBRATION");
    for (unsigned int i = 0; i < 11; i++) {
        auto coeffXml = calibrationXml.appendChild("COEFF"+ofToString(i));
        coeffXml.set(x(i, 0));
    }
    xml.save(path);
...
void ofxKinectProjectorToolkit::loadCalibration(string path) {
    ...
    */
    ...
    if( !xml.load(path) ) {
        ...
        auto calibrationXml = xml.findFirst("//CALIBRATION");
        
        for (unsigned int i = 0; i < 11; i++) {
            float coeff = calibrationXml.getChild("COEFF"+ofToString(i)).getFloatValue();
            x(i, 0) = coeff;
            cout << "x(" << i << ", 0) " << coeff << "\n";
        }
        
        cout << "calibration loaded " << path << "\n";
